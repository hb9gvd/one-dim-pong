...
static inline uint8_t button_is_down(uint8_t pin)
{
  ...
  case PIN_COIN:    return !debtmr_cn && !bstate_cn;
	}
	return 0;
}
...
{
	digitalWrite(PIN_LED_LS, !(l & BL_LS) || lockout_l);
	digitalWrite(PIN_LED_LP, !(l & BL_LP) || zone_l <= 1);
	digitalWrite(PIN_LED_RS, !(l & BL_RS) || lockout_r);
...
//  Serial.println(CREDIT); //prints CREDIT
    ...
    events |= do_debounce(tdiff, &bstate_rp, &debtmr_rp, PIN_COIN,   EV_COIN);
